// ╔══════════════════════════════════════════════╗
// ║   COMANDO: !promote                          ║
// ║   Torna o membro mencionado administrador    ║
// ╚══════════════════════════════════════════════╝
...
module.exports = {
  ...
  descricao: '(ADM) Torna o membro mencionado administrador do grupo',
  ...
  async executar(sock, from, Info, args) {
    try {
      ...
      if (!isAdmin && !isDono) {
        return sock.sendMessage(from, { text: '❌ Apenas *administradores* podem usar o *!promote*.' }, { quoted: Info });
      }
      ...
      if (!botEhAdmin) {
        return sock.sendMessage(from, { text: '🤖 Preciso ser *administrador* do grupo pra promover alguém!' }, { quoted: Info });
      }
      ...
      if (alvos.length === 0) {
        return sock.sendMessage(
          from,
          { text: `❌ Marque a mensagem da pessoa ou use *@menção*.\n\nExemplo: *${config.PREFIXO}promote @fulano*` },
          { quoted: Info }
        );
      }

      await sock.groupParticipantsUpdate(from, alvos, 'promote');
      await sock.sendMessage(
        from,
        { text: `⬆️ Membro(s) promovido(s) a *administrador*!`, mentions: alvos },
        { quoted: Info }
      );
    } catch (err) {
      console.error('❌ Erro no comando !promote:', err);
      await sock.sendMessage(from, { text: '❌ Ocorreu um erro ao tentar promover o membro.' }, { quoted: Info });
    }
  },
};
